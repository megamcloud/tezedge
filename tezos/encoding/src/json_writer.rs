...
impl JsonWriter {
    pub fn new() -> JsonWriter {
        JsonWriter { data: String::with_capacity(1024) }
    }
    ...
    fn push_str(&mut self, value: &str) {
        self.data.push('"');
        self.data.push_str(&value.replace("\"", "\\\"").replace("\n", "\\n").replace("\r", "\\r"));
        self.data.push('"');
    }
    ...
    fn encode_value(&mut self, value: &Value, encoding: &Encoding) -> Result<(), Error> {
        match encoding {
            ...
            Encoding::Sized(_, sized_encoding) => {
                self.encode_value(value, sized_encoding)
            }
